...
from app.models import Chat, ChatParticipant, Message, User, utcnow, db
...
class ChatRepository(BaseRepository):
    ...
    def get_participations(cls, user_id: int) -> list[ChatParticipant]:
        return ChatParticipant.query.filter_by(user_id=user_id) \
            .options(joinedload(ChatParticipant.user)).all()

    @classmethod
    def get_other_participants(cls, chat_ids: list[int],
                               user_id: int) -> list[ChatParticipant]:
    ...
    @classmethod
    def get_other_participant(cls, chat_id: int,
                              user_id: int) -> ChatParticipant | None:
        return ChatParticipant.query.filter(
    ...
    def get_my_chat_ids(cls, user_id: int) -> list[int]:
        return [cp.chat_id for cp in
                ChatParticipant.query.filter_by(user_id=user_id).all()]

    @classmethod
    def find_common_chat(cls, my_chat_ids: list[int],
                         target_user_id: int) -> ChatParticipant | None:
        if not my_chat_ids:
            return None
        return ChatParticipant.query.filter(
    ...
    @classmethod
    def add_message(cls, chat_id: int, author_id: int,
                    text: str = '', image: Optional[str] = None) -> Message:
        msg = Message(
            chat_id=chat_id, author_id=author_id,
    ...
    @classmethod
    def get_unread_counts(cls, chat_ids: list[int],
    ...
    @classmethod
    def get_message_updates(cls, chat_id: int, after_id: int,
                            since_dt) -> list[Message]:
