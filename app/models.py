...
from sqlalchemy import DateTime, Index, Text, UniqueConstraint, event
...
class User(db.Model, UserMixin, SerializerMixin):
    ...
    followers: Mapped[list["Follow"]] = relationship(
        foreign_keys="Follow.followed_id", back_populates="followed", lazy="dynamic", viewonly=True
    )
    following: Mapped[list["Follow"]] = relationship(
        foreign_keys="Follow.follower_id", back_populates="follower", lazy="dynamic", viewonly=True
    )
    ...
    def is_online(self):
        ...
        return delta.total_seconds() < 30  # 30 секунд
...
class Post(db.Model, SerializerMixin):
    ...
    likes: Mapped[list["Like"]] = relationship(
        back_populates="post", cascade="all, delete-orphan", lazy="dynamic"
    )
    comments: Mapped[list["Comment"]] = relationship(
...
class SystemEvent(db.Model, SerializerMixin):
    ...
    category: Mapped[str] = mapped_column(
        default="system"
    )  # push / db / auth / chat / upload / system
...
class Notification(db.Model, SerializerMixin):
    ...
    post_id: Mapped[int] = mapped_column(
        db.ForeignKey("posts.id", ondelete="SET NULL"), nullable=True
    )
...
class Message(db.Model, SerializerMixin):
    ...
    edited_at: Mapped[datetime.datetime | None] = mapped_column(DateTime, nullable=True)
