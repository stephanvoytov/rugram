...
def _register_user(client: FlaskClient, username: str = 'alice',
...
def _login(client: FlaskClient, username_or_email: str = 'alice',
           password: str = 'secret123') -> None:
    """Log in with follow_redirects. User must already exist."""
    client.post('/login', data={
        'email_or_username': username_or_email, 'password': password,
...
class TestPostLifecycle:
    ...
    def test_create_and_view_post(self, client: FlaskClient):
        ...
        r = client.post('/create', data={'text': 'Integration test post!'},
                        follow_redirects=True)
        assert r.status_code == 200
    ...
    def test_like_then_unlike(self, client: FlaskClient):
        ...
        r = client.post('/post/1/like', content_type='application/json',
                        data='{}')
        assert r.status_code == 200
        data = r.get_json()
    ...
    def test_comment_on_post(self, client: FlaskClient):
        ...
        r = client.post('/post/1/comment', data={'text': 'Nice post!'},
                        follow_redirects=True)
        assert r.status_code == 200
    ...
    def test_edit_own_post(self, client: FlaskClient):
        ...
        r = client.post('/edit_post/1', data={'text': 'Updated text'},
                        follow_redirects=True)
        assert r.status_code == 200
