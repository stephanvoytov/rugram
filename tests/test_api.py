...
class TestApiGetPosts:
    ...
    def test_get_posts_pagination(self, client: FlaskClient, app: Flask) -> None:
        """API returns paginated posts with correct structure."""
        _login(client, username="poster")
        for i in range(3):
            client.post("/create", data={"text": f"api post {i}"}, follow_redirects=True)
...
class TestApiGetSinglePost:
    ...
    def test_get_single_post(self, client: FlaskClient, app: Flask) -> None:
        """GET /api/v1/posts/<id> returns the post."""
        _login(client, username="apiuser")
        client.post("/create", data={"text": "single api post"}, follow_redirects=True)
...
class TestApiCreatePost:
    ...
    def test_create_post_via_api(self, client: FlaskClient, app: Flask) -> None:
...
class TestApiDeletePost:
    ...
    def test_delete_post_via_api(self, client: FlaskClient, app: Flask) -> None:
        """DELETE /api/v1/posts/<id> deletes owned post."""
        _login(client, username="api_deleter")
        client.post("/create", data={"text": "api deletable"}, follow_redirects=True)
        with app.app_context():
    ...
    def test_delete_others_post_via_api(self, client: FlaskClient, app: Flask) -> None:
        """DELETE /api/v1/posts/<id> for another's post returns 403."""
        _login(client, username="author_a")
        client.post("/create", data={"text": "a's post"}, follow_redirects=True)
        with app.app_context():
