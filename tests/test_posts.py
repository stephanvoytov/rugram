...
def _create_post(client: FlaskClient, text: str = 'hello world') -> dict:
    ...
    r = client.post('/create', data={'text': text, 'image': ''},
...
class TestCreatePost:
    ...
    def test_create_post_success(self, client: FlaskClient,
                                 app: Flask) -> None:
        """Logged-in user can create a post."""
        _login(client)
        r = client.post('/create', data={'text': 'my first post'},
    ...
    def test_create_post_with_tags(self, client: FlaskClient,
                                   app: Flask) -> None:
        """Post text with #hashtags extracts tags."""
        _login(client)
        client.post('/create', data={'text': 'hello #world #flask'},
                    follow_redirects=True)
        with app.app_context():
...
class TestViewPost:
    def test_view_post(self, client: FlaskClient, app: Flask) -> None:
        ...
        _login(client)
        client.post('/create', data={'text': 'viewable post'},
                    follow_redirects=True)
        with app.app_context():
    ...
    def test_view_deleted_post_404(self, client: FlaskClient,
                                   app: Flask) -> None:
...
class TestEditPost:
    def test_edit_own_post(self, client: FlaskClient, app: Flask) -> None:
        ...
        r = client.post(f'/edit_post/{p.id}',
    ...
    def test_edit_others_post_403(self, client: FlaskClient,
                                  app: Flask) -> None:
        """Non-author cannot edit a post."""
        _login(client, username='author1')
        client.post('/create', data={'text': "author1's post"},
                    follow_redirects=True)
        with app.app_context():
...
class TestDeletePost:
    def test_delete_own_post(self, client: FlaskClient, app: Flask) -> None:
        ...
        client.post('/create', data={'text': 'post to delete'},
                    follow_redirects=True)
        with app.app_context():
            p = Post.query.filter_by(text='post to delete').first()
        r = client.delete(f'/delete/{p.id}',
    ...
    def test_delete_others_post_403(self, client: FlaskClient,
                                    app: Flask) -> None:
        """Non-author cannot delete a post."""
        _login(client, username='author_a')
        client.post('/create', data={'text': "author_a's post"},
                    follow_redirects=True)
        with app.app_context():
...
class TestLike:
    def test_like_post(self, client: FlaskClient, app: Flask) -> None:
        ...
        client.post('/create', data={'text': 'likable post'},
                    follow_redirects=True)
        with app.app_context():
            p = Post.query.filter_by(text='likable post').first()
        r = client.post(f'/post/{p.id}/like',
                        headers={'Content-Type': 'application/json'})
        assert r.status_code == 200
        data = json.loads(r.data)
        assert data.get('status') == 'liked'
        with app.app_context():
            assert Like.query.filter_by(user_id=p.author_id,
                                        post_id=p.id).first() is not None
    ...
    def test_unlike_post(self, client: FlaskClient, app: Flask) -> None:
        ...
        client.post('/create', data={'text': 'unlikable post'},
                    follow_redirects=True)
        with app.app_context():
        ...
        with app.app_context():
            assert Like.query.filter_by(user_id=p.author_id,
                                        post_id=p.id).first() is None

    def test_like_requires_auth(self, client: FlaskClient,
                                app: Flask) -> None:
        """Anonymous user cannot like a post."""
        _login(client, username='poster')
        client.post('/create', data={'text': 'lonely post'},
                    follow_redirects=True)
        with app.app_context():
...
class TestComment:
    def test_add_comment(self, client: FlaskClient, app: Flask) -> None:
        ...
        client.post('/create', data={'text': 'commentable post'},
                    follow_redirects=True)
        with app.app_context():
            p = Post.query.filter_by(text='commentable post').first()
        r = client.post(f'/post/{p.id}/comment',
                        data={'text': 'nice post!'},
                        follow_redirects=True)
        assert r.status_code == 200
        with app.app_context():
    ...
    def test_comment_json_response(self, client: FlaskClient,
                                   app: Flask) -> None:
        """Comment with AJAX header returns JSON."""
        _login(client, username='ajaxer')
        client.post('/create', data={'text': 'ajax post'},
                    follow_redirects=True)
        with app.app_context():
    ...
    def test_delete_own_comment(self, client: FlaskClient,
                                app: Flask) -> None:
        """Author can delete their comment."""
        _login(client, username='deleter')
        client.post('/create', data={'text': 'post with comment'},
                    follow_redirects=True)
        with app.app_context():
            p = Post.query.filter_by(text='post with comment').first()
        # Add comment
        client.post(f'/post/{p.id}/comment', data={'text': 'my comment'},
                    follow_redirects=True)
        with app.app_context():
            c = Comment.query.filter_by(post_id=p.id).first()
        # Delete comment
...
class TestRepost:
    def test_repost_toggle(self, client: FlaskClient, app: Flask) -> None:
        ...
        client.post('/create', data={'text': 'repostable post'},
                    follow_redirects=True)
        with app.app_context():
            p = Post.query.filter_by(text='repostable post').first()
        # Repost
        r = client.post(f'/post/{p.id}/repost',
                        headers={'Content-Type': 'application/json'})
        assert r.status_code == 200
        data = json.loads(r.data)
        assert data.get('status') == 'reposted'
        with app.app_context():
            assert Repost.query.filter_by(user_id=p.author_id,
                                          post_id=p.id).first() is not None
...
class TestSave:
    def test_save_toggle(self, client: FlaskClient, app: Flask) -> None:
        ...
        client.post('/create', data={'text': 'saveable post'},
                    follow_redirects=True)
        with app.app_context():
            p = Post.query.filter_by(text='saveable post').first()
        # Save
        r = client.post(f'/post/{p.id}/save',
                        headers={'Content-Type': 'application/json'})
        assert r.status_code == 200
        data = json.loads(r.data)
        assert data.get('status') == 'saved'
        with app.app_context():
            assert SavedPost.query.filter_by(
                user_id=p.author_id, post_id=p.id).first() is not None
        # Unsave
        r = client.post(f'/post/{p.id}/save',
                        headers={'Content-Type': 'application/json'})
        data = json.loads(r.data)
