...
def _create_user(client: FlaskClient, username: str = 'alice',
...
def _create_user_directly(db, username: str = 'alice',
                          email: str = 'alice@x.com',
                          password: str = 'secret123') -> User:
...
def _login(client: FlaskClient, username: str = 'alice',
           password: str = 'secret123') -> None:
    client.post('/login', data={
        'email_or_username': username, 'password': password,
    }, follow_redirects=True)
...
class TestAuthBypass:
    ...
    def test_protected_post_redirects_to_login(self, client: FlaskClient):
        ...
        for url, data, ct in self.PROTECTED_POST:
            kwargs = {}
            if data:
                kwargs['data'] = data if isinstance(data, str) else data
            if ct:
...
class TestIDOR:
    ...
    def test_post_idor(self, client: FlaskClient, db):
        ...
        r = client.post('/edit_post/1', data={'text': 'Hacked!'},
                        follow_redirects=True)
    ...
    def test_edit_others_comment_403(self, client: FlaskClient, db):
        ...
        self._setup_two_users(db)
        _login(client, 'alice', 'pass1234')
        client.post('/post/1/comment', data={'text': 'Alice comment'},
                    follow_redirects=True)
        _logout(client)
        _login(client, 'bob', 'bobspass')
        r = client.post('/comment/1/edit',
    ...
    def test_chat_idor(self, client: FlaskClient, db):
        ...
        alice_id, bob_id = self._setup_two_users(db)
        _login(client, 'alice', 'pass1234')
        r = client.post('/chat/start/bob')
        assert r.status_code == 200
        chat_id = r.get_json().get('chat_id')
        client.post(f'/chat/{chat_id}/send',
        ...
        assert r.status_code == 403
        # Eve cannot send
        r = client.post(f'/chat/{chat_id}/send',
...
class TestXSS:
    ...
    def test_xss_in_comment(self, client: FlaskClient):
        ...
        r = client.post('/post/1/comment', data={'text': script},
                        follow_redirects=True)
        assert r.status_code == 200
        content = r.data.decode('utf-8')
    ...
    def test_xss_in_profile(self, client: FlaskClient):
        ...
        r = client.post('/edit_profile', data={'description': script},
                        follow_redirects=True)
        assert r.status_code == 200
        # View profile
...
class TestUploadSecurity:
    ...
    def test_invalid_file_extension(self, client: FlaskClient):
        ...
        r = client.post('/create', data=data, follow_redirects=True,
