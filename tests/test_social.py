...
class TestFollow:
    ...
    def test_unfollow_user(self, client: FlaskClient, app: Flask) -> None:
        ...
        # Follow
        client.post('/follow/untarget',
                    headers={'Content-Type': 'application/json'})
        # Unfollow
        r = client.post('/follow/untarget',
                        headers={'Content-Type': 'application/json'})
        data = json.loads(r.data)
    ...
    def test_follow_self_fails(self, client: FlaskClient) -> None:
        ...
        r = client.post('/follow/narcissist',
                        headers={'Content-Type': 'application/json'})
    ...
    def test_follow_nonexistent_user_404(self, client: FlaskClient) -> None:
        ...
        _login(client)
        r = client.post('/follow/nobody',
                        headers={'Content-Type': 'application/json'})
        assert r.status_code in (400, 404)

    def test_follow_redirects_when_anonymous(self,
                                              client: FlaskClient) -> None:
        """Anonymous user cannot follow."""
        r = client.post('/follow/someone',
                        headers={'Content-Type': 'application/json'})
        assert r.status_code in (302, 401)


class TestFollowersPage:
    def test_followers_page_loads(self, client: FlaskClient,
                                  app: Flask) -> None:
    ...
    def test_following_page_loads(self, client: FlaskClient,
                                  app: Flask) -> None:
...
class TestEditProfile:
    ...
    def test_edit_profile_logged_in(self, client: FlaskClient,
                                    app: Flask) -> None:
...
class TestNotifications:
    ...
    def test_unread_count(self, client: FlaskClient, app: Flask) -> None:
        ...
        with app.app_context():
            ...
            db.session.flush()
            n = Notification(user_id=u.id, actor_id=actor.id,
                             type='follow')
            db.session.add(n)
            db.session.commit()
