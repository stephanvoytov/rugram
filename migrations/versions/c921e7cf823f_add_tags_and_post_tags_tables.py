...
def upgrade() -> None:
    """Create tags and post_tags tables (idempotent — skips if already exist)."""
    conn = op.get_bind()
...
def downgrade() -> None:
    """Drop tags and post_tags tables (idempotent)."""
    conn = op.get_bind()
    inspector = inspect(conn)
    existing = inspector.get_table_names()

    if 'post_tags' in existing:
