...
Rugram — социальная сеть с мессенджером, push-уведомлениями, лайками, комментариями и полным управлением аккаунтом.
...
### Уведомления
- **Push-уведомления** — через Service Worker + Push API (VAPID), приходят даже когда сайт закрыт
  - Push при новом сообщении в чате (имя + текст)
  - Push при лайке, комментарии, подписке
  - Автоматическое удаление просроченных подписок
...
- **Кнопка «Написать» в профиле** — автоматическое создание/открытие чата с пользователем
- **Адаптивный дизайн** — mobile-first, master-detail на чатах
...
**Backend:** Python 3.12, Flask, SQLAlchemy, Flask-Login, Flask-WTF, SQLite, cryptography (Fernet), pywebpush  
**Frontend:** Jinja2, Bootstrap 5.3, Bootstrap Icons, чистый CSS/JS, Service Worker, Push API
